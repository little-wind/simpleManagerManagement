  ...
  class Templates {
    ...
    public function display($file) {
      ...
      if(!file_exists($parFile) OR filemtime($parFile) < filemtime($tplFile)) {
        //引用模板解析类
        require_once ROOT_PATH . '\\inc\\Parser.class.php';
    ...
    //create()方法，用于解析不需要创建cache的部分
    //如：header.inc.php，footer.inc.php
    //这些在index.php中调用，可直接生成index.html
    public function createParserFile($file) {
      //创建模板路径
      $tplFile = TPL_DIR . $file;
      ...
      if(!file_exists($parFile) OR filemtime($parFile) < filemtime($tplFile)) {
        //引用模板解析类
        require_once ROOT_PATH . '\\class\\Parser.class.php';
        $Parser = new Parser($tplFile);
        $Parser->compile($parFile);
      }
